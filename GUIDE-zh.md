...
- [Airwallex Flutter Plugin](#airwallex-flutter-plugin)
  ...
  - [创建PaymentSession](#创建paymentsession)
    - [配置GooglePayOptions/ApplePayOptions](#配置googlepayoptionsapplepayoptions)
    - [配置returnUrl](#配置returnurl)
      - [Android：](#android-1)
      - [iOS：](#ios)
...
```
之后正常运行 `flutter pub get` 与 `flutter build ios` 即可，Flutter 会自动通过 SPM 解析 iOS 依赖。如需切换回 CocoaPods，请执行 `flutter config --no-enable-swift-package-manager`。
...
> `GooglePayOptions` 和 `ApplePayOptions` 对于所有 session 类型都是可选的。
> 请注意在 `RecurringSession` 和 `RecurringWithIntentSession` 中 Google Pay 和 Apple Pay 仅支持 `nextTriggeredBy: merchant`。
